...
const QUESTION = 'Have you ever had driving experience with other apps before?*';
...
export default function QuestionnaireScreen() {
    ...
    const handleContinue = async () => {
        ...
        try {
            const res = await fetch(
                `${Config.API_URL}/riders/registrations/${encodeURIComponent(regId)}/questionnaire`,
                {
                    method: 'PATCH',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ hadDrivingExperienceOtherApps: value }),
                }
            );
            if (!res.ok) {
    ...
    const displayText = value === true ? 'Yes' : value === false ? 'No' : 'Yes / No';
    const displayPlaceholder = value === null;
    ...
    return (
        <SafeAreaView style={s.safeArea} edges={['top']}>
            <View style={s.header}>
                ...
                <Text style={s.headerTitle}>Questionnaire</Text>
                <View style={{ width: 24 }} />
            </View>
            ...
            >
                ...
                <Text style={s.title}>Questionnaire</Text>
                ...
                <TouchableOpacity
                    ...
                    activeOpacity={0.8}
                >
                    <Text style={[s.selectText, displayPlaceholder && s.selectPlaceholder]}>
                        {displayText}
                    </Text>
                    <Ionicons
                ...
                {showDropdown && (
                    <View style={s.dropdown}>
                        <TouchableOpacity
                            ...
                            activeOpacity={0.7}
                        >
                            <Text style={s.optionText}>Yes</Text>
                        </TouchableOpacity>
                        ...
                        <TouchableOpacity
                            ...
                            activeOpacity={0.7}
                        >
                            <Text style={s.optionText}>No</Text>
                        </TouchableOpacity>
                    </View>
                )}
            </ScrollView>
