...
export default function VerifyDocumentsScreen() {
  ...
  const pickFile = async () => {
    try {
      ...
      Alert.alert('Error', 'Could not open file picker. You can try selecting a photo from the gallery instead.');
      const { status } = await ImagePicker.requestMediaLibraryPermissionsAsync();
      if (status !== 'granted') {
        Alert.alert('Permission required', 'Please allow access to photos to upload your document.');
        return;
      }
    ...
    <SafeAreaView style={s.safeArea} edges={['top']}>
      ...
      >
        ...
        <View style={s.infoBanner}>
          ...
          <View style={s.infoTextWrap}>
            ...
            <Text style={s.infoSub}>Please ensure that the information in your documents is up to date.</Text>
          </View>
        </View>
      ...
      <View style={[s.footer, { paddingBottom: Math.max(24, insets.bottom + 12) }]}>
        <TouchableOpacity
          ...
          activeOpacity={0.85}
        >
          {loading ? <ActivityIndicator color="#FFFFFF" /> : <Text style={s.submitText}>Continue</Text>}
        </TouchableOpacity>
      </View>
    </SafeAreaView>
  );
