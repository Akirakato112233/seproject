...
export default function DriverLicenseScreen() {
    ...
    const handleContinue = async () => {
        if (!data.selfieUri) {
            Alert.alert('กรุณาถ่ายรูปเซลฟี่', 'โปรดถ่ายและอัปโหลดรูปเซลฟี่ของคุณก่อนดำเนินการต่อ', [
        ...
        if (licenseDigits.length !== 8) {
            Alert.alert('ข้อมูลไม่ถูกต้อง', 'เลขใบขับขี่ต้องเป็นตัวเลข 8 หลัก');
            return;
        }
        if (!licenseType.trim()) {
            Alert.alert('กรุณากรอกข้อมูล', 'กรุณากรอกชนิดใบขับขี่');
            return;
        }
        if (!issueDate) {
            Alert.alert('กรุณากรอกข้อมูล', 'กรุณาเลือกวันอนุญาต');
            return;
        }
        if (!expiryDate) {
            Alert.alert('กรุณากรอกข้อมูล', 'กรุณาเลือกวันหมดอายุ');
            return;
        }
        if (!province.trim()) {
            Alert.alert('กรุณากรอกข้อมูล', 'กรุณากรอกจังหวัดออกใบขับขี่');
            return;
        }
        if (!data.licenseUri) {
            Alert.alert('กรุณาอัปโหลดรูป', 'กรุณาอัปโหลดรูปใบขับขี่');
            return;
        }
        ...
        if (result.success) {
            ...
            Alert.alert(
                'เกิดข้อผิดพลาด',
                result.message ?? 'ไม่สามารถบันทึกข้อมูลได้ กรุณาลองใหม่'
            );
        }
    };
                ...
                >
                    ...
                    <View style={s.titleRow}>
                        ...
                        <TouchableOpacity
                            style={s.uploadBox}
                            onPress={() =>
                                router.push('/signup/upload-doc-guide?type=license' as any)
                            }
                        >
                            {data.licenseUri ? (
                                <Image
                                    source={{ uri: data.licenseUri }}
                    ...
                    {licenseNo.length > 0 && licenseDigits.length < 8 && (
                        <Text style={s.warningHint}>
                            กรอกให้ครบ 8 หลัก ({licenseDigits.length}/8)
                        </Text>
                    )}
