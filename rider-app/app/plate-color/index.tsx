...
const OPTIONS = [
  { value: 'white' as const, label: 'ป้ายขาว' },
  { value: 'green' as const, label: 'ป้ายเขียว' },
  { value: 'yellow' as const, label: 'ป้ายเหลือง' },
  { value: 'red' as const, label: 'ป้ายแดง' },
];

export default function PlateColorScreen() {
  const router = useRouter();
  const { registrationId } = useLocalSearchParams<{ registrationId?: string }>();
  const insets = useSafeAreaInsets();
  const [value, setValue] = useState<'white' | 'green' | 'yellow' | 'red' | null>(null);
  ...
  const handleContinue = async () => {
    ...
    try {
      const res = await fetch(
        `${Config.API_URL}/riders/registrations/${encodeURIComponent(regId)}/plate-color`,
        {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ plateColor: value }),
        },
      );
      if (!res.ok) {
  ...
  return (
    <SafeAreaView style={s.safeArea} edges={['top']}>
      <View style={s.header}>
        ...
        <Text style={s.headerTitle}>รายละเอียดเพิ่มเติม</Text>
        <View style={{ width: 24 }} />
      </View>
      ...
      >
        ...
        <Text style={s.title}>รายละเอียดเพิ่มเติม</Text>
        <Text style={s.question}>ประเภทป้ายทะเบียนรถ (สีป้ายทะเบียน)*</Text>
        ...
        >
          <Text style={[s.selectText, !selected && s.selectPlaceholder]} numberOfLines={1}>
            {selected ? selected.label : 'เลือกสีป้ายทะเบียน'}
          </Text>
          <Ionicons
...
const s = StyleSheet.create({
  ...
  question: { fontSize: 16, color: '#334155', marginBottom: 12 },
  selectWrap: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
